...
import {v6PreflightScan,v6SelectDestination,v6AfterScan} from "../server/v6/qr-runtime.js";
...
export default async function handler(request,response){
  try{
    ...
    const context=contextFromRequest(request),preflight=await v6PreflightScan(db,link,shortId,request,context),destination=await v6SelectDestination(link,context,()=>selectDestination(link,context));if(!safeUrl(destination))return sendHtml(response,400,shell("Destination unavailable","<h1>Destination unavailable</h1><p>This QR does not currently have a valid destination.</p>"));
    ...
    if(preflight.recordAnalytics)await recordScan(db,shortId,link,context,request);await v6AfterScan(db,link,preflight,{country:context.country,city:context.city,device:context.device});return response.redirect(302,destination);
